        ...
        <div class="col-md-12 grid-margin">
            <div class="row">
                <div class="col-12 col-xl-12 mb-4 mb-xl-0">
                    <h3 class="font-weight-bold">Deposit</h3>
                    <h6 class="font-weight-normal mb-3">Manage deposit here</h6>
                </div>
            </div>
...
<script type="text/javascript">
        ...
        $("#dt_entries").DataTable({
            ...
            "columns": [{
                    "mRender": function(data, type, row) {
                        return row.status == 'F' ? '' : "<input type='checkbox' value=" + row.deposit_id + " class='dt_id' style='position: initial; opacity:1;'>";
                    }
                },
                {
                    "mRender": function(data, type, row) {
                        return "<center><button class='btn btn-primary btn-circle btn-sm' onclick='getEntryDetails2(" + row.deposit_id + ")'><span class='ti ti-list'></span></button></center>";
                    }
                },
                {
                    "data": "deposit_date"
    ...
    function getEntries2() {
        var params = "deposit_id = '" + $("#hidden_id_2").val() + "'";
        ...
        $("#dt_entries_2").DataTable({
            ...
            "columns": [{
                    "mRender": function(data, type, row) {
                        return "<input type='checkbox' value=" + row.deposit_detail_id + " class='dt_id_2' style='position: initial; opacity:1;'>";
                    }
                },
                {
                    "data": "remarks"
    ...
    function depositType(type){
        if(type == "H"){
            $("#div_check").show();
            $("#div_amount").hide();
            $("#cp_id").prop('required', true);
            $("#amount").prop('required', false);
            getSelectOption('Deposit', 'cp_id', 'reference', "payment_type='H' AND status='F' AND deposit_status=0");
        }else{
            $("#div_check").hide();
            $("#div_amount").show();
            $("#cp_id").prop('required', false);
