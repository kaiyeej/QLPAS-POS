...
class Menus extends Connection
{
    ...
    public function routes($page, $dir)
    {
        ...
        if ($page == 'homepage' || $page == 'profile') {
            ...
            if ($has_page) {
                ...
                if ($UserPrivileges->check($page, $_SESSION['jcis_user']['id']) == 1) {
                    $this->dir = $dir;
                    $this->route_settings = [
    ...
    {
        $UserPrivileges = new UserPrivileges();
        if ($UserPrivileges->check($url, $_SESSION['jcis_user']['id']) == 1) {
    ...
    public function sidebar_parent($name, $ti, $child)
    {
        ...
        foreach ($child as $row) {
            if ($UserPrivileges->check($row[1], $_SESSION['jcis_user']['id']) == 1) {
