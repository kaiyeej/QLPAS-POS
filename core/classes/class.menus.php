...
class Menus extends Connection
{
    public function lists()
    {
        $this->menus = array(
            ...
            'report' => array(
                ...
                array('url' => 'count-sheet', 'name' => 'Countsheet', 'class_name' => 'InventoryReport', 'has_detail' => 0),
    ...
    public function routes($page, $dir)
    {
        ...
        if ($page == 'homepage' || $page == 'profile') {
            ...
            if ($has_page) {
                ...
                if ($UserPrivileges->check($page, $_SESSION['user_grocery']['id']) == 1) {
                    $this->dir = $dir;
                    $this->route_settings = [
    ...
    {
        $UserPrivileges = new UserPrivileges();
        if ($UserPrivileges->check($url, $_SESSION['user_grocery']['id']) == 1) {
    ...
    public function sidebar_parent($name, $ti, $child)
    {
        ...
        foreach ($child as $row) {
            if ($UserPrivileges->check($row[1], $_SESSION['user_grocery']['id']) == 1) {
